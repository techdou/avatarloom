...
// 指向容器服务名（http://control-api:8100/api / http://runtime-gateway:8101/api）。
// 与 lib/api.ts 的 CONTROL_API_BASE 约定一致（带 /api 后缀）。
const CONTROL_API = process.env.CONTROL_API_BASE ?? 'http://127.0.0.1:8100/api';
const REALTIME_API = process.env.RUNTIME_GATEWAY_BASE ?? 'http://127.0.0.1:8101/api';
