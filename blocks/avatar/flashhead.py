...
class FlashHeadAvatarBlock(Block):
    ...
    async def setup(self, ctx: BlockContext) -> None:
        ...
        self._load_portrait(cfg, workspace)

        # 默认路径可用环境变量覆盖（AVATARLOOM_FLASHHEAD_*），降低非 AutoDL 环境启动成本
        service_python, service_script, model_dir, wav2vec_dir = (
            self._resolve_service_config(cfg, workspace)
        )
        port = int(cfg.get("servicePort", 8767))
        jpeg_quality = int(cfg.get("jpegQuality", 85))

        # 路径预检：缺失时明确报错（而不是 spawn 失败难定位）
        self._validate_service_paths(service_python, service_script, model_dir, wav2vec_dir)
        ...
        ws = await self._connect_service_ws(websockets, ws_url, log_path, log_file)
        if ws is None:
            await self._stop()
            log_file.close()
    ...
    def _load_portrait(self, cfg: dict[str, Any], workspace: Path) -> None:
        ...
        portrait_path = Path(portrait_cfg)
        if not portrait_path.is_absolute():
            portrait_path = workspace / portrait_path
        if not portrait_path.exists():
            raise BlockSetupError("avatar.flashhead", f"portrait not found: {portrait_path}")
        self._portrait_path = str(portrait_path)
        self._portrait_bytes = portrait_path.read_bytes()
    ...
    def _resolve_service_config(
        ...
        return service_python, service_script, model_dir, wav2vec_dir
    ...
    async def _perform_handshake(self, ws: Any) -> None:
        ...
        await ws.send(json.dumps({"type": "set_image", "path": self._portrait_path}))
    ...
    async def process(self, ctx: BlockContext, event: Event) -> None:
        ...
        if changed:
            await self._send_state_transition(ctx, new_state)
        self._avatar_state = new_state

        if ev_type == TTS_AUDIO_DELTA:
            await self._forward_audio(ctx, event)
        elif ev_type == TTS_AUDIO_COMPLETED:
            await self._handle_tts_completed(ctx)

    async def _send_state_transition(self, ctx: BlockContext, new_state: Any) -> None:
        if self._ws is None:
            return
        try:
            if new_state.speech_active != self._avatar_state.speech_active:
                await self._ws.send(
                    json.dumps({"type": "speech_active", "on": new_state.speech_active})
                )
            if new_state.idle_mode != self._avatar_state.idle_mode:
                await self._ws.send(
                    json.dumps({"type": "idle_mode", "mode": new_state.idle_mode})
                )
        except Exception as e:
            await ctx.logger.aerror("flashhead state send failed", error=str(e))

    async def _forward_audio(self, ctx: BlockContext, event: Event) -> None:
        pcm_b64 = event.payload.get("pcm_b64", "")
        if not pcm_b64 or self._ws is None:
            return
        try:
            await self._ws.send(json.dumps({"type": "audio", "pcm": pcm_b64}))
        except Exception as e:
            await ctx.logger.aerror("flashhead send audio failed", error=str(e))

    async def _handle_tts_completed(self, ctx: BlockContext) -> None:
        ...
        try:
            async for message in self._ws:
                ...
                if isinstance(message, (bytes, bytearray)):
                    await self._handle_frame_message(ctx, message)
                else:
                    await self._handle_control_message(ctx, message)
        except Exception as e:
            if ctx is not None:
    ...
    async def _handle_frame_message(
        ...
        if not jpeg:
            return
        await self._emit_jpeg_frame(ctx, jpeg, is_speech=tag == 0x01)

    async def _handle_control_message(self, ctx: BlockContext, message: Any) -> None:
        # JSON 控制帧（error 等）
        try:
